...
def fetch(offset_date, ref):
    ...
    r = requests.get(
        url, params=params, timeout=30, headers={"User-Agent": "GHActionFetcher/1.0"}
    )
    r.raise_for_status()
    return r.text
...
def parse_table(html):
    ...
    div = soup.find("div", {"id": "roboblock"}) or soup.find(
        "div", {"class": "roboblock"}
    )
    if not div:
...
def main():
    ...
    print(
        "Wrote roboblock.html and roboblock.json (rows=",
        len(parsed.get("rows", [])),
        ")",
    )
